...
def retrieval_from_trainset( retrieval_term, a, u ,trainset_input_vectors, topk = 5,  erase_same_idx=True, ref_number = 1 ):
        num = a.shape[0]
        ...
        for i in tqdm.tqdm( range( num ) ):
            vector_i = retrieval_term[i].reshape( 1, -1 )
            ...
            avg_score /= topk
            pair_a.append( { "x": a[i], "ref_x": np.array( refer_sets ), "avg_score": avg_score } )
            co_u.append( u[i] )
...
def get_rag_dataloader(xtrain, ytrain, xtest, ytest, batch_size, rag_configs, train_shuffle = True):
    ...
    trainset_input_vectors  = xtrain.reshape(xtrain.shape[0], -1)
    trainset = ( xtrain, ytrain )

    pair_train_a, co_train_u, _ = retrieval_from_trainset( xtrain, xtrain, ytrain, trainset_input_vectors,  training_refer_range , erase_same_idx=True, ref_number = refer_num )
    ...
    pair_test_a, co_test_u, _ = retrieval_from_trainset(xtest,  xtest, ytest, trainset_input_vectors,  1 ,  erase_same_idx=False , ref_number= refer_num )
