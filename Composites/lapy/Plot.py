...
def plot_tet_mesh(tetra, vfunc=None, plot_edges=None, plot_levels=False, tfunc=None,
                  ...
                  xrange=None, yrange=None, zrange=None, showcaxis=False, caxis=None, export_png=None, camera=None,no_display = False,
                  vmin = None, vmax = None, export_pdf=False):
    ...
    return plot_tria_mesh(tria_bnd, vfunc=vfunc, plot_edges=plot_edges, plot_levels=plot_levels,
                   tfunc=tfunc_tria, edge_color=edge_color, html_output=html_output,
                   width=width, height=height, flatshading=flatshading,
                   xrange=xrange, yrange=yrange, zrange=zrange, showcaxis=showcaxis, caxis=caxis, export_png= export_png, camera=camera, no_display=no_display,
                   vmin = vmin, vmax = vmax, export_pdf=export_pdf)


def plot_tria_mesh(tria, vfunc=None, tfunc=None, vcolor=None, tcolor=None,
                   showcaxis=False, caxis=None, xrange=None, yrange=None,
                   zrange=None, plot_edges=None, plot_levels=False,
                   edge_color='rgb(50,50,50)', tic_color='rgb(50,200,10)',
                   background_color=None, flatshading=False, view=None,
                   width=800, height=800, camera=None, html_output=False,
                   export_png=None, scale_png=1.0, no_display=False,
                   vmin = None, vmax = None, export_pdf=False):
    ...
    if (vfunc is not None or tfunc is not None) and (vcolor is not None or tcolor is not None):
        raise ValueError('plot_tria_mesh can only use either vfunc/tfunc or vcolor/tcolor, but not both at the same time')

    if vcolor is not None and tcolor is not None:
        raise ValueError('plot_tria_mesh can only use either vcolor or tcolor, but not both at the same time')
    ...
    if vfunc is None:
        if tfunc is None:
            triangles = go.Mesh3d(x=x, y=y, z=z, i=i, j=j, k=k, flatshading=flatshading, vertexcolor=vcolor, facecolor=tcolor)
        ...
        if plot_levels:
            ...
            colorscale = _get_colorscale(vmin, vmax)
    ...
    fig.update_layout( margin=dict(  b=0, l = 0, pad=0, r = 0, t = 0 ) )
    ...
    if export_png is not None:
        ...
        if export_pdf:
            fig.write_image(export_png.replace( '.png', '.pdf' ), scale=scale_png)
